...
export default async function ArticlesAdminPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  return (
    <>
      ...
      <section className="admin-card admin-table-wrap"><table className="admin-table"><thead><tr><th>Story</th><th>Status</th><th>Author</th><th>Updated</th><th>Views</th><th /></tr></thead><tbody>{rows.map((article) => <tr key={article.id}><td><strong>{article.title}</strong><small>{humanize(article.type)} · /{article.slug}</small></td><td><span className={`admin-status ${article.status.toLowerCase()}`}>{humanize(article.status)}</span></td><td>{article.authorName ?? "Unknown"}</td><td>{new Date(article.updatedAt).toLocaleDateString()}</td><td>{article.views.toLocaleString()}</td><td><Link href={`/admin/articles/${article.id}`}>Edit</Link></td></tr>)}</tbody></table>{rows.length === 0 ? <p className="admin-empty">No articles match this view.</p> : null}</section>
